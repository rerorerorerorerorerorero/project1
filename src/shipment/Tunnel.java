   ...
   public synchronized boolean add(Ship element){
        try{
            if (shipsCounter < MAX_SHIPS_IN_TUNNEL){
                notifyAll();
                store.add(element);
                String info = String.format("%s + The ship arrived in the tunnel %s, %s, %s", store.size(), element.getType(), element.getSize(), element.getCount());
                System.out.println(info);
                shipsCounter++;
            }else {
                System.out.println(store.size()+ "> There is no space for ship in the tunnel:"+ Thread.currentThread().getName());
                wait();
                return false;
            }
